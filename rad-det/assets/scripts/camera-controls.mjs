...
class CameraControls extends Script {
    ...
    /**
     * The scene boundary box. Camera movement will be constrained within this box.
    ...
    /**
     * Clamp a position to be within the scene boundary.
     * Also prevents sliding along walls by zeroing out movement components that hit boundaries.
     *
     * @param {Vec3} newPosition - The new position to clamp.
     * @param {Vec3} oldPosition - The previous position before movement.
     * @returns {Vec3} - The clamped position.
     */
    clampToBoundary(newPosition, oldPosition) {
        if (!this.sceneBoundary || !this.sceneBoundary.min || !this.sceneBoundary.max) {
            return newPosition;
        }

        const min = this.sceneBoundary.min;
        const max = this.sceneBoundary.max;

        // Check each axis independently and prevent movement along walls
        if (newPosition.x < min[0]) {
            newPosition.x = min[0];
            // If we hit the boundary, stop movement in this direction
            if (oldPosition.x <= min[0]) {
                newPosition.x = oldPosition.x;
            }
        } else if (newPosition.x > max[0]) {
            newPosition.x = max[0];
            if (oldPosition.x >= max[0]) {
                newPosition.x = oldPosition.x;
            }
        }

        if (newPosition.y < min[1]) {
            newPosition.y = min[1];
            if (oldPosition.y <= min[1]) {
                newPosition.y = oldPosition.y;
            }
        } else if (newPosition.y > max[1]) {
            newPosition.y = max[1];
            if (oldPosition.y >= max[1]) {
                newPosition.y = oldPosition.y;
            }
        }

        if (newPosition.z < min[2]) {
            newPosition.z = min[2];
            if (oldPosition.z <= min[2]) {
                newPosition.z = oldPosition.z;
            }
        } else if (newPosition.z > max[2]) {
            newPosition.z = max[2];
            if (oldPosition.z >= max[2]) {
                newPosition.z = oldPosition.z;
            }
        }
    ...
    update(dt) {
        ...
        const previousPosition = this._camera.entity.getPosition().clone();
        
        // update controller by consuming frame
        this._pose.copy(this._controller.update(frame, dt));
        
        // Clamp position to scene boundary (prevents sliding along walls)
        this.clampToBoundary(this._pose.position, previousPosition);

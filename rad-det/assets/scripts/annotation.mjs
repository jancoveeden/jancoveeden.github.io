...
export class Annotation extends Script {
    ...
    /**
     * The camera position to move to when the annotation is clicked.
     *
     * @type {Vec3}
     * @attribute
     */
    ...
    /**
     * The camera target (look at point) to move to when the annotation is clicked.
     *
     * @type {Vec3}
     * @attribute
     */
     ...
     */
    static createHotspotTexture(app, alpha = 0.8, size = 64, fillColor = '#2c222aff', strokeColor = '#da0a0aff', borderWidth = 6) {
    ...
    initialize() {
        ...
        // Expose Annotation class globally for reset functionality
        ...
        this._hotspot.addEventListener('click', (e) => {
            ...
            if (Annotation._activeTooltip === this._tooltip) {
                ...
                if (window.showAnnotationInPanel) {
                    const details = {
                        description: this.text || 'No additional information available'
                    };
                    window.showAnnotationInPanel(this.title, details);
                }

                // Move camera to predefined position if specified
                if (this.cameraPosition) {
                    this._moveCameraToPosition(this.cameraPosition, this.cameraTarget);
                }
            }
        });
    ...
    /**
     * Move the camera to a predefined position and target.
     * @param {Vec3} position - The camera position to move to
     * @param {Vec3} target - The target point to look at
     * @private
     */
    _moveCameraToPosition(position, target) {
        // Find the camera controls script
        const cameraEntity = this.app.root.findByName('camera');
        if (!cameraEntity) {
            console.warn('Annotation: camera entity not found');
            return;
        }

        const cameraControls = cameraEntity.script?.cameraControls;
        if (!cameraControls) {
            console.warn('Annotation: cameraControls script not found');
            return;
        }

        // If target is not provided, use the annotation's world position
        const focusTarget = target || this.entity.getPosition();

        // Update the focus point to the annotation target
        cameraControls.focusPoint = focusTarget;

        // Use the reset method to smoothly move to the new position
        cameraControls.reset(focusTarget, position);
    }
